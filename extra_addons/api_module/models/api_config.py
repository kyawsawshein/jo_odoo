...
class ApiConfig(models.Model):
    ...
    graphql_url = fields.Char(
        string="GraphQL URL", required=True, default="http://localhost:8000/graphql"
    )
    ...
    jwt_secret = fields.Char(string="JWT Secret Key", help="Secret key for JWT token verification")
    ...
    issuer = fields.Char(string="JWT ISSUER Key", help="Issuer key for JWT token verification")
    audience = fields.Char(string="JWT AUDIENCE Key", help="Audience key for JWT token verification")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 401:
                    ...
                    error_msg += ": Server error. Please check if the API service is running."
                else:
            ...
            raise UserError(f"Request timeout: The API server took too long to respond. Check if the service is running and accessible.")
    ...
    def execute_graphql_query(self, query, variables=None):
        ...
        _logger.info(f"URL : {self.graphql_url}, Payload : {payload} , Header : {headers}")
        try:
